...
// Хеш-функция, которая вычисляет индекс в хеш-таблице для заданного ключа
int HashFunction(int key, int size, int p)
{
    return (p + key) % size;
}

// Функция для вычисления следующего хеша при коллизии
...
{
    // Создание нового объекта хеш-таблицы с указанным размером и функцией получения ключа
    return *(new Object(size, getkey));
}

// Реализация конструктора класса Object
Object::Object(int size, int(*getkey)(void*))
{
    N = 0; // Инициализация количества элементов в таблице
    ...
    for (int i = 0; i < size; ++i)
        data[i] = NULL; // Инициализация всех указателей NULL
}
