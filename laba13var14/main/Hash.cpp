#include "Hash.h" // Включение заголовочного файла с объявлением класса Object
#include <iostream>
...
{
    // Линейное пробирование: при коллизии добавляется константа к текущему хешу, а затем берется остаток от деления на размер таблицы
    return (hash + p) % size;
}
