...
int countNodes(Tree* p);           // Количество вершин в дереве
...
double averageOfValues(Tree* p); // Среднее арифметическое всех элементов дерева
Tree* Root = NULL;  // Указатель на корень

int main()
{
    setlocale(0, ""); // Убрал указание языка, чтобы использовать локаль по умолчанию
    int key, choice, n;
    Tree* rc;
    char s[15];
    for (;;)
    {
        ...
        cout << "11: Выход\n";
        cout << "12: Среднее арифметическое всех элементов дерева\n"; // New option for average
        cout << "Введите: ";
        cin >> choice;
        cout << "\n";
        switch (choice)
        ...
        case 12: // Option to calculate and display the average of all elements in the tree
            double average = averageOfValues(Root);
            cout << "Среднее арифметическое всех элементов дерева: " << average << endl;
            break;
        }
    }
}
...
double averageOfValues(Tree* p) {
    int sum = sumOfValues(p); // Reuse the sumOfValues function to get the total sum
    int count = countNodes(p); // Count the total number of nodes
    if (count == 0) // Handle division by zero
        return 0.0;
    return static_cast<double>(sum) / count; // Return the average as a double
}
